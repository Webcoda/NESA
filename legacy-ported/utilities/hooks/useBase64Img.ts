...
export default function (): [(content: string) => string, Record<string, string | null>] {
  const [encodingMap, setEncodingMap] = useState<Record<string, string | null>>({});

  return [
    (content: string) => {
      // Replace any known images with encoded version
      const replaced = Object.entries(encodingMap).reduce((acc, [urlRegex, encoding]) => {
        if (encoding) {
          const replacement = acc.replace(new RegExp(urlRegex, 'gi'), encoding);
      ...
      const matches = replaced.matchAll(/<img .*?src=['"]((?!data)[^"]+)['"]/g);
      if (matches) {
