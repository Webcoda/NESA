...
import { IStage, ISyllabus, RowValue } from './backendTypes'
...
function isTreeState(state: CustomSyllabusSplitState | CustomSyllabusTreeState): state is CustomSyllabusTreeState {
	return Object.prototype.hasOwnProperty.call(state, 'stageLearningAreas')
}

export const customSyllabusQueryString = (state: CustomSyllabusSplitState | CustomSyllabusTreeState) => {
	...
	const stageLearningAreas = stageIds.map<StageWithSyllabusIds>((stageId) => ({
		stageId,
		syllabusIds,
	}))
...
export const queryStringComponentAsArray = (qs: undefined | string | string[] | ParsedQs | ParsedQs[]): string[] => {
	if (Array.isArray(qs)) {
		return qs as string[]
	}
	if (typeof qs === 'object') {
...
export const stripHtml = (value: string): string => stripTags(value).replace(/&nbsp;/g, ' ')

const nonAlphaNumRegex = /[^a-zA-Z0-9]+/g
export const uriConvert = (value: string): string => value.replace(nonAlphaNumRegex, '-').toLowerCase()
 ...
 * @param html
 */
export const corsSafeImgs = (html: string): string => html.replace(/img src/g, 'img crossorigin="anonymous" src')
