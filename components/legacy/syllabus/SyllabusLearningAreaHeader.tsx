...
const SyllabusLearningAreaHeader = (props: SyllabusLearningAreaHeaderProps): JSX.Element => {
  const { subjectTag, subjectTitle, selectedAreas, area, onLearningAreaHeaderConfirm } = props;

  const [displayLearningAreaModal, setDisplayLearningAreaModal] = useState(false);
  ...
  if (syllabusHeaderButtonRef) {
    useFocusTabIndex(displayLearningAreaModal, syllabusHeaderButtonRef.current);
  }
      ...
      <div className="syllabus-header__body">
        <Grid container item sm={12} alignItems="center">
          ...
          <div className="syllabus-header__titles">
            ...
            >
              <h3 className="syllabus-header__main-title">{subjectTitle}</h3>
              <ExpandMoreIcon />
            </button>
          </div>
        </Grid>
