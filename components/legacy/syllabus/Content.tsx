...
const Content = (props: ContentOrganizerProps): JSX.Element => {
  const { defaultOffsetTop, stageId, supportElementId, content, initialState, files } = props;

  const mainBodyRef = useRef<HTMLDivElement>(null);

  const [currentContentOrganiser, setCurrentContentOrganiser] = useState<IContents>();
  ...
  const isEarlyStage1 = stageId === Stages.earlyStage1.id;
  ...
  useEffect(() => {
    ...
    if (content?.length && !organiser) {
      if (!initialContentSelect && initialState?.contentOrganiser) {
        ...
        organiser = content.find((c) => c.content_organiser === initialState.contentOrganiser);
      } else {
          ...
          <Grid item xs={12} md={6} lg={4}>
            {content?.map((organiser, index) => (
              <div
                ...
                key={`outcome-card-${organiser.code}-${index}`}
                className="content-organizer__outcome-card-wrapper"
              >
                <span id={`outcome-card-${organiser.code}`} />
                <OutcomeCard
                  title={organiser.content_organiser}
                  code={organiser.outcomes.map((o) => o.key)}
                  outcomes={organiser.outcomes.map((o) => o.value)}
                  ...
                  currentContentOrganiser === organiser && (
                    ...
                    >
                      <OutcomeDetailCard
                        ...
                        accessPoints={organiser.accessPoints}
