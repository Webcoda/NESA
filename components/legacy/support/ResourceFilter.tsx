...
const ResourceFilter = (props: ResourceFilterProps): JSX.Element => {
  ...
  const handleOptionSelectChange = (event: React.ChangeEvent<{ name: string; value: string }>) => {
    const newFilter: ISelectFilter = { name: event.target.name, value: event.target.value };
    ...
    if (newFilter.value) {
      ...
      const newFilterValue = filterValues.filter((filter) => filter.name !== newFilter.name);
   ...
   */
  useEffect(() => {
    const learningAreaDropdown = filterValues.find((dropdown) => dropdown.name === 'learning-areas');

    if (learningAreaDropdown) {
      const syllabusDropdown = dropdowns.find((dropdown) => dropdown.id === 'syllabus');
      ...
      if (syllabusDropdown) {
        ...
        const newOptions = syllabusDropdown.options.filter((option) => {
          ...
          const learningAreaDropdownValue = learningAreaDropdown.value.toLowerCase();
          return optionText.startsWith(learningAreaDropdownValue);
        });
        const newSyllabusDropdown = { ...syllabusDropdown, options: newOptions };

        /**
         * Replacing new syllabus dropdown in dropdown list
         */
        const foundIndex = [...dropdowns].findIndex((dropdown) =>
          dropdown.id === newSyllabusDropdown.id);
  ...
  return (
    <div className="resource-filter">
      <Grid container className="resource-filter__select-group" spacing={2}>
        {filterDropdowns.map((dropdown) => (
          <Grid key={dropdown.id} item xs={12} sm={dropdown.gridSize || 4}>
            <CustomSelect
              name={dropdown.id}
              label={dropdown.name}
